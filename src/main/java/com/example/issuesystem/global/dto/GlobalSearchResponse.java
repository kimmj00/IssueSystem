 ...
 *
 * 변경 사항:
 * - 기존에는 패치이력/지식공유 결과를 최대 50건까지만 내려줬다.
 * - 이제 패치이력 결과와 지식공유 결과를 각각 페이징할 수 있도록 페이지 메타 정보를 함께 내려준다.
 */
@Getter
@Builder
public class GlobalSearchResponse {
    ...
    /** 패치이력 전체 검색 결과 수 */
    private long patchHistoryTotal;

    /** 지식공유 DB 전체 검색 결과 수 */
    private long knowledgeTotal;

    /** 패치이력 + 지식공유 전체 검색 결과 수 */
    private long total;

    /** 현재 페이지에 표시할 패치이력 결과 목록 */
    private List<GlobalSearchItemResponse> patchHistories;
    ...
    /** 패치이력 결과 페이지당 표시 개수 */
    private int patchHistorySize;

    /** 패치이력 결과 전체 페이지 수 */
    private int patchHistoryTotalPages;

    /** 패치이력 결과 다음 페이지 존재 여부 */
    private boolean patchHistoryHasNext;

    /** 패치이력 결과 이전 페이지 존재 여부 */
    private boolean patchHistoryHasPrevious;
